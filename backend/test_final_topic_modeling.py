#!/usr/bin/env python3
"""
Updated test to validate the advanced topic modeling implementation
...
def safe_extract_keywords(topic):
    """Safely extract keywords from topic data regardless of format"""
    try:
        if 'keywords' in topic and isinstance(topic['keywords'], list) and len(topic['keywords']) > 0:
            if isinstance(topic['keywords'][0], str):
                # New format: keywords are strings
                return ', '.join(topic['keywords'][:5])
            else:
                # Old format: keywords are tuples
                return ', '.join([f"{word}({score:.3f})" for word, score in topic['keywords'][:5]])
        elif 'top_words' in topic and isinstance(topic['top_words'], list):
            # Use top_words with scores
            return ', '.join([f"{word}({score:.3f})" for word, score in topic['top_words'][:5]])
        else:
...
def test_lda_algorithm():
    ...
    if response.status_code == 200:
        ...
        # Display topics using safe extraction
        topics = data.get('topics', [])
        for i, topic in enumerate(topics):
            keywords = safe_extract_keywords(topic)
...
def validate_requirements_compliance(lda_result: Dict, nmf_result: Dict, comparison_result: Dict = None):
    ...
    for result, name in [(lda_result, 'LDA'), (nmf_result, 'NMF')]:
        if result and result.get('model_performance') and result['model_performance']:
    ...
    bonus_features = []
    for result, name in [(lda_result, 'LDA'), (nmf_result, 'NMF')]:
        if result and result.get('coherence_score') is not None:
            bonus_features.append(f"{name} coherence scoring")
        if result and result.get('model_performance', {}).get('training_time'):
            bonus_features.append(f"{name} performance tracking")
    
    if bonus_features:
        compliance_checks.append(f"✅ Advanced Features: {', '.join(bonus_features)}")
...
def main():
    ...
    try:
        ...
        # Test algorithm comparison (optional)
